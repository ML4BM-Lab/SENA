...
class Norman2019DataLoader:
    def __init__(self, num_gene_th=5, dataname="Norman2019_raw"):
...
def build_activity_score_df(model, adata, ptb_targets):
    ...
    for int_type in ptb_targets + [""]:  # +control
        ...
        int_df = pd.DataFrame(
            model.encoder(torch.tensor(obs).float().to("cuda")).detach().cpu().numpy()
        )
...
def compute_layer_weight_contribution(
    model, adata, knockout, ens_knockout, num_affected, mode="sena", only_exp=False
):
    ...
    knockout_exp_mean = (
        adata[adata.obs["guide_ids"] == knockout].X[:, idx].todense().mean()
    )
...
def compute_activation_df(
    model, adata, gos, scoretype, mode, gene_go_dict, ensembl_genename_dict, ptb_targets
):
    ...
    for knockout in na_activity_score.keys():
        ...
        if mode[:4] == "sena":
            belonging_genesets = [
                geneset
                for geneset in gos
                if geneset in gene_go_dict[ensembl_genename_dict[knockout]]
            ]
        else:
            belonging_genesets = compute_layer_weight_contribution(
                model, adata, knockout, ens_knockout, 100, mode=mode
            )
        ...
        for i, geneset in enumerate(gos):
            ...
            if scoretype == "ttest":
                _, p_value = ttest_ind(
                    ctrl_cells[:, i], knockout_cells[:, i], equal_var=False
                )
            ...
            if mode[:4] == "sena":
                ttest_df.append(
                    [knockout, geneset, scoretype, score, geneset in belonging_genesets]
                )
            elif mode[:7] == "regular" or mode[:2] == "l1":
                ttest_df.append(
                    [knockout, i, scoretype, score, i in belonging_genesets]
                )
    ...
    if scoretype == "mu_diff":
        for knockout in ttest_df["knockout"].unique():
            ttest_df.loc[
                ttest_df["knockout"] == knockout, "score"
            ] = MinMaxScaler().fit_transform(
                ttest_df.loc[ttest_df["knockout"] == knockout, "score"].values.reshape(
                    -1, 1
                )
            )
...
def compute_outlier_activation_analysis(ttest_df, mode="sena"):
    ...
    if ttest_df["scoretype"].iloc[0] == "ttest":
        ...
        ttest_df["score"] = (
            ttest_df["score"] * ttest_df.shape[0]
    ...
    for knockout in ttest_df["knockout"].unique():
        ...
        score_affected = knockout_distr.loc[
            knockout_distr["affected"] == True, "score"
        ].values.mean()
        median_affected = knockout_distr["score"].median()

        """second test - recall at 25/100"""
        recall_at_100 = (
            knockout_distr.sort_values(by="score", ascending=False)["affected"]
            .iloc[:100]
            .sum()
            / knockout_distr["affected"].sum()
        )
        recall_at_25 = (
            knockout_distr.sort_values(by="score", ascending=False)["affected"]
            .iloc[:25]
        ...
        ##append
        outlier_activation.append(
            [knockout, score_affected, median_affected, recall_at_100, recall_at_25]
        )

    """append results into dataframe"""
    outlier_activation_df = pd.DataFrame(outlier_activation)
    outlier_activation_df.columns = [
        "knockout",
        "top_score",
        "median_score",
        "recall_at_100",
    ...
    paired_pval = ttest_ind(
        outlier_activation_df.iloc[:, 1].values, outlier_activation_df.iloc[:, 2].values
    ).pvalue
    z_diff = (
        outlier_activation_df.iloc[:, 1].values
        - outlier_activation_df.iloc[:, 2].values
    ).mean()

    # append to dict
    outlier_activation_dict = {
        "mode": mode,
        "-log10(pval)": -1 * m.log10(paired_pval),
        "z_diff": z_diff,
        "scoretype": ttest_df["scoretype"].iloc[0],
...
def compute_sparsity_contribution(model, dataset, mode, sparsity_th=[1e-6]):
    ...
    with torch.no_grad():
        ...
        for i in range(dmean.shape[0]):
            contributions.append(
                (dmean[i] * layer_weights[i, :]).detach().cpu().numpy()
            )

    contr_mat = np.vstack(contributions)
    sp_dict = {}
    for sth in sparsity_th:
        sparsity = (np.abs(contr_mat) <= sth).sum() / (
            contr_mat.shape[0] * contr_mat.shape[1]
        )
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(
            torch.empty((self.output_genes, self.input_gs), **factory_kwargs)
        )
