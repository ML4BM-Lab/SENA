...
def plot_sparsity_analysis(mode = '1layer', methods = [], dataset = 'norman', structure='ae'):
    ...
    df = build_dataset()
    sparsity_thresholds = [x for x in df.columns if 'sparsity' in x]
    ...
    sns.barplot(x='Sparsity Threshold', y='Sparsity', hue='mode', data=grouped_sorted, errorbar='sd', palette=color_mapping, capsize=0.1)
