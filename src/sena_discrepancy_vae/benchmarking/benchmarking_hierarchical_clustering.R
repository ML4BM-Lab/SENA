...
modeltype <- "full_go_sena_delta_0"
layername <- "z"
mode <- 'absdm'
norm <- TRUE
if (norm){
    file_path <- paste0("./result/uhler/",modeltype,"/seed_42/activation_scores_",mode,"_DEA_layer_",layername,"_norm_matrix.tsv")
}else{
    file_path <- paste0("./result/uhler/",modeltype,"/seed_42/activation_scores_",mode,"_DEA_layer_",layername,"_matrix.tsv")
}
...
if (norm){
    png(filename = paste0("./figures/uhler/",modeltype,"/seed_42/activation_scores_layer_",
                          layername,"_heatmap_",mode,"_th_",toString(th),"_norm.png"), width = 1000, height = 1000)
}else{
    png(filename = paste0("./figures/uhler/",modeltype,"/seed_42/activation_scores_layer_",
                          layername,"_heatmap_",mode,"_th_",toString(th),".png"), width = 1000, height = 1000)
}
